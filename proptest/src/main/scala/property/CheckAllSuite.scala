...
trait CheckAllSuite extends FunSuite {
     ...
     */
    def checkAll[A](name: String)(gen: Generator[A])(f: A => Unit)
        (using config: PropTestConfig)
        (using context: PropertyContext = PropertyContext(name), location: Location): PropertyContext =
        PropertyTest(name, gen)(f)
